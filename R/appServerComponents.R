  ...
  datasetInformationServer <- ({
   # Add Log Tool Tips
    ...
    observeEvent(input$inputDataType, {
      if (input$inputDataType == "Upload") {
        # Add Gene Expression File Upload Button
        output$dataInput <- renderUI({
          fileInput("fileUpload", "Upload Gene Expression Dataset",
                    multiple = TRUE,
                    accept = c("text/csv",
        ...
        # Add Experimental Conditions File Upload Button
        output$dataInput2 <- renderUI({
          fileInput("fileUpload2", "Upload Experimental Conditions Dataset",
                    multiple = TRUE,
                    accept = c("text/csv",
                               "text/comma-separated-values,text/plain",
                               ".csv"))
        ...
        expressionData <- reactive({
          try({readxl::read_excel(inut$fileUpload)})
        })

      } else if (input$inputDataType == "GEO") {
        # Add GEO Accession Code Input
        output$dataInput <- renderUI({
        ...
        # Add Platform Input
        output$dataInput2 <- renderUI({
          selectInput("platform", "Select the platform of interest.", c())
        })
